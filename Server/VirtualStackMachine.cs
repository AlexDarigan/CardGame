...
namespace CardGame.Server
{
    public class VirtualStackMachine
    {
        ...
        private void Push(bool i)
        {
            Push(Convert.ToInt32(i));
        }

        private Instructions Next(int i)
        {
            return (Instructions) _stack[i];
        }

        private void GetCards(IEnumerable<Card> zone)
        {
            _cards.AddRange(zone);
        }
        ...
        private void If(int jumpToElseBranch, bool condition)
        {
            _index = condition ? _index : jumpToElseBranch;
        }

        private void SetValue(Action<Card, int> setter, int value)
        {
            _cards.ForEach(card => setter(card, value));
        }
