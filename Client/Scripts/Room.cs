...
namespace CardGame.Client
{
    ...
    public class Room : Node
    {
        ...
        {
            _commandQueue.Enqueue((Command) Call(commandId.ToString(), args));
        }
        ...
        {
            _cards[id].Update(state);
        }

        [Puppet]
        private void Deploy(Card card)
        {
            RpcId(Server, "Deploy", card.Id);
        }

        [Puppet]
        private void Set(Card card)
        {
            RpcId(Server, "Set", card.Id);
        }
        ...
        private Command LoadDeck(bool isClient, Dictionary<int, SetCodes> deck)
        {
            return new LoadDeck(GetPlayer(isClient), deck, _cards.GetCard);
        }

        private Command Draw(bool isClient, int cardId)
        {
            return new Draw(GetPlayer(isClient), GetCard(cardId));
        }

        private Command Deploy(bool isClient, int cardId)
        {
            return new Deploy(GetPlayer(isClient), GetCard(cardId));
        }
        ...
        private Participant GetPlayer(bool isClient)
        {
            return isClient ? _player : _rival;
        }

        private Card GetCard(int id, SetCodes setCode = SetCodes.NullCard)
        {
            return _cards.GetCard(id, setCode);
        }
    }
}