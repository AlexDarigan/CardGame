...
namespace CardGame.Tests.Fixtures
{
    public class ClientFixtures: WAT.Test
    {
        ...
        public override async void Pre()
        {
            ...
            P1 = (Participant) _room1.Get("_player");
            P2 = (Participant) _room2.Get("_player");
            await Update();
        }
        
        public override void Post() { GameScene.Free(); }
        protected async Task Update(float time = 2.5f) => await ToSignal(UntilSignal(GameScene, nameof(Main.RoomsUpdated), time), YIELD);
    }
}